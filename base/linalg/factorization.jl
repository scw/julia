...
cholfact{T<:BlasFloat}(A::StridedMatrix{T}, uplo::Symbol=:U; pivot=false, tol=-1.0) = cholfact!(copy(A), uplo, pivot=pivot, tol=tol)
cholfact{T}(A::StridedMatrix{T}, uplo::Symbol=:U; pivot=false, tol=-1.0) = (S = promote_type(typeof(sqrt(one(T))),Float32); S != T ? cholfact!(convert(Matrix{S},A), uplo, pivot=pivot, tol=tol) : cholfact!(copy(A), uplo, pivot=pivot, tol=tol)) # When julia Cholesky has been implemented, the promotion should be changed.
...
# Note. For QRWY factorization without pivoting, the WY representation based method introduced in LAPACK 3.4
immutable QRWY{S<:BlasFloat} <: Factorization{S}
    factors::Matrix{S}
    T::Matrix{S}
end
...
qrfact!{T<:BlasFloat}(A::StridedMatrix{T}; pivot=false) = pivot ? QRPivoted{T}(LAPACK.geqp3!(A)...) : QRWY(LAPACK.geqrt!(A, min(minimum(size(A)), 36))...)
...
convert{T}(::Type{QRWY{T}},A::QRWY) = QRWY(convert(Matrix{T}, A.factors), convert(Matrix{T}, A.T))
...
function getindex(A::QRWY, d::Symbol)
    d == :R && return triu(A.factors[1:minimum(size(A)),:])
    d == :Q && return QRWYQ(A.factors,A.T)
    throw(KeyError(d))
end
...
type QRPackedQ{T} <: AbstractMatrix{T}
    factors::Matrix{T}
    τ::Vector{T}
end
type QRWYQ{S} <: AbstractMatrix{S} 
    factors::Matrix{S}                      
    T::Matrix{S}                       
end

convert{T,S}(::Type{QRPackedQ{T}}, Q::QRPackedQ{S}) = QRPackedQ(convert(Matrix{T}, Q.factors), convert(Vector{T}, Q.τ))
convert{S1,S2}(::Type{QRWYQ{S1}}, Q::QRWYQ{S2}) = QRWYQ(convert(Matrix{S1}, Q.factors), convert(Matrix{S1}, Q.T))

size(A::Union(QR,QRWY,QRPivoted), dim::Integer) = size(A.factors, dim)
size(A::Union(QR,QRWY,QRPivoted)) = size(A.factors)
size(A::Union(QRPackedQ,QRWYQ), dim::Integer) = 0 < dim ? (dim <= 2 ? size(A.factors, 1) : 1) : throw(BoundsError())
size(A::Union(QRPackedQ,QRWYQ)) = size(A, 1), size(A, 2)

full{T}(A::Union(QRPackedQ{T},QRWYQ{T}); thin::Bool=true) = A_mul_B!(A, thin ? eye(T, size(A.factors)...) : eye(T, size(A.factors,1)))

print_matrix(io::IO, A::Union(QRPackedQ,QRWYQ), rows::Integer, cols::Integer) = print_matrix(io, full(A, thin=false), rows, cols)

## Multiplication by Q
### QB
A_mul_B!{T<:BlasFloat}(A::QRWYQ{T}, B::StridedVecOrMat{T}) = LAPACK.gemqrt!('L','N',A.factors,A.T,B)
    ...
    @inbounds begin
        for k = min(mA,nA):-1:1
            for j = 1:nB
                νBj = B[k,j]
                for i = k+1:mB
                    νBj += conj(A.factors[i,k])*B[i,j]
                end
                νBj *= conj(A.τ[k])
                B[k,j] -= νBj
                for i = k+1:mB
                    B[i,j] -= A.factors[i,k]*νBj
                end
            end
        end
    end
...
function *{TA,Tb}(A::Union(QRPackedQ{TA},QRWYQ{TA}), b::StridedVector{Tb})
...
function *{TA,TB}(A::Union(QRPackedQ{TA},QRWYQ{TA}), B::StridedMatrix{TB})
    TAB = promote_type(TA,TB)
    Anew = convert(typeof(A).name.primary{TAB},A)
...
### QcB
Ac_mul_B!{T<:BlasReal}(A::QRWYQ{T}, B::StridedVecOrMat{T}) = LAPACK.gemqrt!('L','T',A.factors,A.T,B)
Ac_mul_B!{T<:BlasComplex}(A::QRWYQ{T}, B::StridedVecOrMat{T}) = LAPACK.gemqrt!('L','C',A.factors,A.T,B)
...
function Ac_mul_B{TQ<:Number,TB<:Number}(Q::Union(QRPackedQ{TQ},QRWYQ{TQ}), B::StridedVecOrMat{TB})
    TQB = promote_type(TQ,TB)
    Ac_mul_B!(convert(typeof(Q).name.primary{TQB}, Q), TB == TQB ? copy(B) : convert(typeof(B).name.primary{TQB}, B))
end
### AQ
A_mul_B!{T<:BlasFloat}(A::StridedVecOrMat{T}, B::QRWYQ{T}) = LAPACK.gemqrt!('R','N', B.factors, B.T, A)
...
function *{TA,TQ}(A::StridedVecOrMat{TA}, Q::Union(QRPackedQ{TQ},QRWYQ{TQ}))
    TAQ = promote_type(TA, TQ)
    A_mul_B!(TA==TAQ ? copy(A) : convert(typeof(A).name.primary{TAQ}, A), convert(typeof(Q).name.primary{TAQ}, Q))
end
### AQc
A_mul_Bc!{T<:BlasReal}(A::StridedVecOrMat{T}, B::QRWYQ{T}) = LAPACK.gemqrt!('R','T',B.factors,B.T,A)
A_mul_Bc!{T<:BlasComplex}(A::StridedVecOrMat{T}, B::QRWYQ{T}) = LAPACK.gemqrt!('R','C',B.factors,B.T,A)
...
function A_mul_Bc{TA,TB}(A::AbstractVecOrMat{TA}, B::Union(QRWYQ{TB},QRPackedQ{TB}))
    TAB = promote_type(TA,TB)
...
function A_ldiv_B!{T<:BlasFloat}(A::Union(QRWY{T},QRPivoted{T}), B::StridedMatrix{T}, rcond::Real)
    mA, nA = size(A.factors)
    nr = min(mA,nA)
    nrhs = size(B, 2)
...
A_ldiv_B!{T<:BlasFloat}(A::Union(QRWY{T},QRPivoted{T}), B::StridedVector, rcond::Real) = A_ldiv_B!(A,reshape(B,length(B),1), rcond)
A_ldiv_B!{T<:BlasFloat}(A::Union(QRWY{T},QRPivoted{T}), B::StridedVecOrMat{T}) = A_ldiv_B!(A, B, sqrt(eps(real(float(one(eltype(B)))))))[1]
# A_ldiv_B!(A::QRWY, B::StridedVector) = A_ldiv_B!(Triangular(A[:R], :U), Ac_mul_B(A[:Q], B))[1:size(A, 2)]
# A_ldiv_B!(A::QRWY, B::StridedMatrix) = A_ldiv_B!(Triangular(A[:R], :U), Ac_mul_B(A[:Q], B))[1:size(A, 2),:]
...
function \{TA,TB}(A::Union(QR{TA},QRWY{TA},QRPivoted{TA}),B::StridedVector{TB})
...
function \{TA,TB}(A::Union(QR{TA},QRWY{TA},QRPivoted{TA}),B::StridedMatrix{TB})
    S = promote_type(TA,TB)
    m,n = size(A)
